...
- **Switch models for cost control.** Two-way auto interpreters: `gpt-realtime-2.1-mini` (cheapest, ~$0.016/min, the default), `gpt-realtime-2.1`, `gpt-realtime`, `gpt-realtime-mini`. Plus OpenAI's dedicated **`gpt-realtime-translate`** speech-to-speech translator. *(The retired `gpt-4o-realtime-preview` models — shut down May 2026 — have been removed.)*
- **Two translation modes.** The two-way models auto-detect which language is spoken and interpret both directions in a single hands-free session. `gpt-realtime-translate` is a dedicated one-way translator (one target language per session), so it shows a **⇄ Switch direction** control you tap when the other person speaks. Note: the translator only *outputs* 13 languages (English, Spanish, Portuguese, French, Italian, German, Russian, Japanese, Korean, Hindi, Indonesian, Vietnamese, Chinese) — for anything else (e.g. Arabic/Darija output) use a two-way model.
...
- **📷 Photo translate.** Take a photo (or pick one) of a sign, menu, label, or document and get the **English** translation on screen — with copy and read-aloud. Uses an OpenAI vision model (default `gpt-5-mini`, configurable, with automatic fallbacks). Images are downscaled in-browser before upload to keep it fast and cheap.
